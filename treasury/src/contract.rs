//! Treasury contract implementation (logic only; the impl with `#[contractimpl]` lives in `lib.rs`).

#[cfg(not(test))]
use crate::clients::GroupContract;
use crate::errors::Error;
use crate::events::{
    Contribution, ContributionWithdrawn, FundRoundCompleted, FundRoundProposed, ReleaseApproved,
    ReleaseCanceled, ReleaseExecuted, ReleaseProposed, TreasuryCreated,
};
...
pub fn create_treasury(
        ...
        user.require_auth();
        crate::helpers::check_membership(&env, group_id, user.clone())?;
...
pub fn propose_release(
        ...
        user.require_auth();
        crate::helpers::check_membership(&env, group_id, user.clone())?;
        crate::helpers::check_membership(&env, group_id, destination.clone())?;
        crate::helpers::assert_treasury_exists(&env, group_id)?;
...
pub fn approve_release(
        ...
        crate::helpers::check_membership(&env, release.group_id, user.clone())?;
...
pub fn execute_release(env: Env, release_proposal_id: u64) -> Result<(), Error> {
        ...
        #[cfg(not(test))]
        let min: u32 = {
            let group_contract = Address::from_str(&env, crate::config::GROUP_CONTRACT);
        ...
        #[cfg(not(test))]
        {
            let usdc_address = Address::from_str(&env, crate::config::USDC_ADDRESS);
...
pub fn contribute_to_fund_round(
        ...
        crate::helpers::assert_treasury_exists(&env, round.group_id)?;
        crate::helpers::check_membership(&env, round.group_id, user.clone())?;
...
pub fn has_sufficient_group_balance(env: Env, group_id: u64, amount: i128) -> bool {
    get_group_balance(env, group_id) >= amount
}
